...
const PartnershipForm = () => {
  ...
  const onSubmit = (data: FormValues) => {
    ...
    const webAppUrl = 'https://script.google.com/macros/s/AKfycbyGUDrwIVYZoezpU78yG0Y7qQSauGScK6_sRj6oICZXZDFqL8RBrVvD_iYMrHzeUmC86Q/exec'; // <--- PASTE YOUR URL HERE
    ...
    const formData = new URLSearchParams({
      ...
      termsAgreed: data.termsAgreed.toString(), // Send boolean as string
      // Exclude signature file data for now, or send a placeholder/link if handled elsewhere
      // signature: data.signature ? 'Signature uploaded' : 'No signature',
      signature: data.signature ? 'Signature uploaded' : 'No signature', // Sending a placeholder text
    }).toString();

    // Send data to Google Apps Script
    fetch(webAppUrl, {
      method: 'POST',
      mode: 'no-cors', // Use no-cors for simple public endpoints
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
      },
      body: formData,
    }).then(() => {
      // This block will be executed regardless of the script's success due to no-cors
      // You can show a success toast here, assuming the fetch itself didn't fail network-wise
      toast({
      ...
      // This catch block will only run for network errors, not script errors in no-cors mode
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="space-y-2">
                  <a 
                    ...
                    className="flex items-center space-x-3 p-3 bg-[#2A2F3C] rounded-lg hover:bg-[#3A3F4C] transition-colors group"
                  >
              ...
              <div>
                ...
                <a 
                  ...
                  target="_blank" 
                  rel="noopener noreferrer" 
                  className="flex items-center space-x-3 p-3 bg-green-600 rounded-lg hover:bg-green-700 transition-colors group"
                >
